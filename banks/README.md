...
### 1. Work from a real export — or say that you did not

Bank exports are full of things no spec mentions: trailing empty columns, subtotal lines,
descriptions spread over three columns or two lines, dates with a clock time on them,
footers that look like data. A profile written from a one-line format description is a
guess that looks like a feature.

A published format sample — the kind open-source import tools carry in their source — is
decent second-hand evidence, and enough to start from. A real file is better. Whichever you
had, **say so at the top of the bank's README**:

```md
> **Provenance:** derived from publicly documented format samples.
> Not yet verified against a real export.
```

and set the column in the root README's table to match. Nobody is harmed by a profile built
on second-hand evidence; someone is harmed by not knowing that is what it is.
