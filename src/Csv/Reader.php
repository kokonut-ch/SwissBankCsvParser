...
final class Reader
{
    ...
    /**
     * Banks write the preamble in one of two ways: the label in the first cell
     * and the value in the second — "Compte:";"CH93…" — or the whole thing in
     * a single cell, "Kontonummer: 543.278.22". Both are read.
     *
     * The single-cell form is only accepted on a row that has nothing else in
     * it. Otherwise an ordinary booking such as "Lastschrift: Grundversicherung"
     * would be filed away as if it were account metadata.
     *
     * @param  list<string>  $row
     * @return array{0: string, 1: string}|null
     */
    private static function metadataPair(array $row): ?array
    {
        $first = trim($row[0] ?? '');

        // Cheapest possible rejection first: this runs on every row of the file,
        // and an ordinary booking line has no colon in its first cell.
        if ($first === '' || ! str_contains($first, ':')) {
            return null;
        }

        if (str_ends_with($first, ':')) {
            $label = rtrim(rtrim($first, ':'));
            $value = trim(Text::unwrapExcelFormula(trim($row[1] ?? '')));

            return $label === '' || $value === '' ? null : [$label, $value];
        }

        $populated = array_filter($row, static fn (string $cell): bool => trim($cell) !== '');

        if (count($populated) !== 1) {
            return null;
        }

        [$label, $value] = explode(':', $first, 2);
        $label = trim($label);
        $value = trim(Text::unwrapExcelFormula(trim($value)));

        // A long left-hand side is prose, not a label.
        if ($label === '' || $value === '' || mb_strlen($label) > 40) {
            return null;
        }
